...
const App: () => Node = () => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        let parsedPairedDevices;
        if (pairedDevices) {
          parsedPairedDevices = JSON.parse(pairedDevices);
          setConnectedDevices(parsedPairedDevices);
        } else {
          // Scan for BLE devices if there are no previously paired devices
          // setShowDialog(true);
        }
      } catch (error) {
        console.error(error);
  ...
  //       } else {
  //         // Scan for BLE devices if there are no previously paired devices
  //         const discoveredDevices = await manager.startDeviceScan(
  //           null,
  //           null,
  //           (error, device) => {
  //             if (error) {
  ...
  const handlePress = () => {
    Linking.openURL(
      'https://www.facebook.com/people/Erri-Fireplaces/100063587050379/',
    );
  };
      ...
      <Modal visible={!isConnected} transparent={true}>
        <ImageBackground
          ...
          <TouchableOpacity
            ...
            <Text style={styles.notConnected}>Scan for Devices</Text>
          </TouchableOpacity>
        </ImageBackground>
      </Modal>
